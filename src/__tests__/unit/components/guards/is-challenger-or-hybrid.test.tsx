import { isChallengerOrHybrid } from '@/components/guards/is-challenger-or-hybrid';
...
describe('isChallengerOrHybrid', () => {
  ...
  it('returns a component that can be rendered.', () => {
    ...
    const TestComponent = isChallengerOrHybrid(function () {
  ...
  it('redirects the user to /actions if they are a player-type user.', () => {
  ...
  it('allows access to a page if the user is signed in as a challenger.', () => {
    const user = Builder<User>().type(UserType.Challenger).build();
    const userContextValue = Builder<UserContextType>().user(user).build();
  ...
  it('redirects the user upon signout.', async () => {
    function MockUserContextProvider({ children }: PropsWithChildren) {
      const [user, setUser] = useState<User | null>(
        Builder<User>().type(UserType.Challenger).build(),
      );
